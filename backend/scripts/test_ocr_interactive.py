...
async def get_or_create_test_user() -> User:
    user, created = await User.get_or_create(
        email="test_ocr_chat@example.com", defaults={"name": "테스트유저", "hashed_password": "dummy"}
    )
    return user
...
async def run_interactive_test(image_path: str):
    ...
    while True:
        ...
        # 유저 메시지 선저장
        await chat_service.save_message(session_id=session.id, user_id=user.id, content=user_input, is_faq=False)
        ...
        try:
            ...
            ai_msg = await chat_service.get_ai_response(
                session_id=session.id, user_id=user.id, user_message=user_input, background_tasks=bg_tasks
            )
            print(f"\n>> {ai_msg.content}\n")
        except Exception as e:
            print(f"\n❌ 챗봇 답변 실패: {e}")
