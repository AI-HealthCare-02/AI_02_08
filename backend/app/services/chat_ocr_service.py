...
class ChatOcrService:
    ...
    def _check_illicit_drugs(self, text: str) -> bool:
        illicit_drugs = ["대마", "코카인", "헤로인", "필로폰", "엑스터시", 
    ...
    def _load_medication_data(self):
        ...
        try:
            ...
            file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "userMedicationData.json")
    ...
    async def process_ocr(self, image_bytes: bytes) -> dict:
        """
        이미지 분석 후 결과 반환
        - 'e약은요'를 불러오지 못할 경우에 대비하여 userMedicationData.json을 활용하여 분석 결과 데이터를 냄
        """
        try:
            # 여기서 e약은요 API 연동 로직을 시도 (현재 미구현/실패 상황을 가정)
...
의학적 조언은 신중하게 하되, 처방전 내용을 우선으로 설명하세요."""
            ...
            # API 키가 없거나 문제 발생시 기본 응답 (chat2.py의 기능 fallback)
