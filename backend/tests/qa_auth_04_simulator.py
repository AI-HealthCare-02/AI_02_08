...
async def test_auth_04():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        resp = await client.post(
            f"{BASE_URL}/api/v1/auth/password/reset-request", json={"email": "non-exist@example.com"}
        )
        print(f"결과 응답: {resp.status_code}, {resp.text}")
        if resp.status_code == 200:
            print(
                "ℹ️ 알림: 실제 구현은 보안상의 이유로(Email Enumeration 방지) 미가입 이메일에도 성공 메시지를 반환합니다."
            )
        else:
            print("❌ 결과: 실패")
        ...
        verification = (
            await EmailVerification.filter(email=test_email, type=VerificationType.PASSWORD_RESET)
            .order_by("-created_at")
            .first()
        )
        ...
        if resp.status_code == 200:
            ...
            login_resp = await client.post(
                f"{BASE_URL}/api/v1/auth/login", json={"email": test_email, "password": new_pass}
            )
