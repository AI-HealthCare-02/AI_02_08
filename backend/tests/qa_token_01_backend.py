...
async def test_token_01_backend():
    ...
    _ = str(tokens["access_token"])
    refresh_token = str(tokens["refresh_token"])

    # Refresh Token DB 저장 (실제 환경 시뮬레이션)
    await RefreshToken.create(user_id=user.id, token=refresh_token, expires_at=datetime.now() + timedelta(days=7))

    async with httpx.AsyncClient(timeout=10.0) as client:
        # 1. 만료된/잘못된 Access Token으로 요청 시 401 확인
        print("\n[TOKEN-01-1-A] 잘못된 Access Token 요청 테스트")
        resp = await client.get(f"{BASE_URL}/api/v1/users/me", headers={"Authorization": "Bearer invalid_token"})
        ...
        resp = await client.get(f"{BASE_URL}/api/v1/auth/token/refresh", cookies=cookies)
        if resp.status_code == 200:
            ...
            resp_me = await client.get(
                f"{BASE_URL}/api/v1/users/me", headers={"Authorization": f"Bearer {new_access_token}"}
            )
        ...
        expired_refresh = jwt_service.create_token(
            data={"sub": str(user.id), "type": "refresh"}, expires_delta=timedelta(seconds=-10)
        )
        resp = await client.get(
            f"{BASE_URL}/api/v1/auth/token/refresh", cookies={"refresh_token": str(expired_refresh)}
        )
        print(f"✅ 결과: {resp.status_code} (예상: 401)")
